...
export const TaskColumn = ({
  ...
  return (
    <div
      className="flex flex-col w-[400px] max-h-[calc(100vh-260px)] h-fit bg-white rounded-[20px] p-5 shadow-sm border border-gray-100/70"
    ...
    >
      <div className="flex items-center justify-between mb-4 px-1">
        <div className="flex items-center gap-3">
          ...
          <span className="flex h-7 w-7 items-center justify-center rounded-full bg-gray-50 text-xs font-bold text-gray-500 border border-gray-100">
            {tasks.length}
          </span>
        </div>
      ...
      <div className="overflow-y-auto no-scrollbar flex flex-col gap-3 px-0.5 pb-2">
        {tasks.map((task) => (
          <TaskCard
            key={task.id}
            task={task}
