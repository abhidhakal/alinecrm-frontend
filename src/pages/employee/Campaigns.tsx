...
export default function Campaigns() {
  ...
  // React Query Hooks
  const { data: campaigns = [], isLoading, refetch } = useGetAllCampaigns();
  const { data: templates = [], isLoading: isLoadingTemplates, refetch: refetchTemplates } = useGetAllTemplates();
  ...
  return (
    <div className="flex min-h-screen w-full bg-white font-sans">
      ...
      <div className={`flex flex-1 flex-col transition-all duration-300 ${isExpanded ? 'ml-[280px] max-w-[calc(100vw-280px)]' : 'ml-[110px] max-w-[calc(100vw-110px)]'}`}>
        <CampaignsHeader onRefresh={refetch} lastUpdated={new Date()} />
          ...
          <div className="mt-8 mb-6">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <button onClick={() => setIsSettingsModalOpen(true)} className="p-2 text-gray-400 hover:text-gray-600 active:scale-95 transition-all">
                  <img src="/icons/settings-icon.svg" alt="Settings" className="h-5 w-5" />
                </button>
                <button onClick={() => setIsCreateModalOpen(true)} className="flex items-center gap-2 rounded-xl bg-black px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-all hover:bg-black/90 active:scale-[0.98]">
                  Create Campaign
                </button>
              </div>
            </div>
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="h-10 w-[300px] rounded-full border border-gray-100 bg-gray-50/50 pl-10 pr-4 text-sm focus:bg-white focus:border-gray-200 focus:ring-4 focus:ring-gray-100 outline-none transition-all"
                  />
                </div>

                <div className="relative">
                  <button onClick={() => setIsStatusDropdownOpen(!isStatusDropdownOpen)} className="flex h-10 items-center gap-2 rounded-full border border-gray-100 bg-white px-4 text-sm font-medium text-gray-700 hover:bg-gray-50">
                      ...
                      <div className="absolute left-0 mt-2 z-20 w-40 rounded-xl border border-gray-100 bg-white py-1 shadow-xl">
                        {['all', 'draft', 'scheduled', 'sending', 'sent', 'failed'].map((status) => (
                          <button
                            ...
                            className={`flex w-full items-center px-4 py-2 text-sm capitalize ${statusFilter === status ? 'bg-gray-50 font-bold' : 'hover:bg-gray-50'}`}
                          >
                            {status}
                          </button>
                        ))}
            ...
            {isLoading ? (
              ...
              filteredCampaigns.map((campaign) => (
                <CampaignCard
                  ...
                  campaign={campaign}
                  onRefresh={refetch}
                  onEdit={(c) => { setEditingCampaign(c); setIsCreateModalOpen(true); }}
        ...
        <CreateCampaignModal
          ...
          onSuccess={() => { refetch(); setIsCreateModalOpen(false); setEditingCampaign(null); }}
