import Header from '../components/Header';

const features = [
  {
    name: 'Pipeline Management',
    description: 'Visualize your sales process with customizable boards. Drag and drop deals to move them forward.',
  },
  {
    name: 'Smart Automation',
    description: 'Trigger emails, tasks, and notifications automatically based on deal stage changes.',
  },
  {
    name: 'Unified Communications',
    description: 'Sync your email and calendar to keep all customer interactions in one place.',
  },
  {
    name: 'Deep Analytics',
    description: 'Gain insights into team performance and revenue forecasts with real-time reports.',
  },
];

export default function Features() {
  return (
    <div className="flex w-full flex-col min-h-screen bg-white font-sans">
      <Header />
      <div className="mx-auto max-w-7xl px-6 lg:px-8 py-24 sm:py-32">
        <div className="mx-auto max-w-2xl lg:text-center">
          <h2 className="text-base font-semibold leading-7 text-indigo-600">Faster Growth</h2>
          <p className="mt-2 text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
            Everything you need to scale your sales
          </p>
          <p className="mt-6 text-lg leading-8 text-gray-600">
            AlineCRM provides the essential tools to turn leads into loyal customers without the complexity of legacy systems.
          </p>
        </div>
        <div className="mx-auto mt-16 max-w-2xl sm:mt-20 lg:mt-24 lg:max-w-4xl">
          <dl className="grid max-w-xl grid-cols-1 gap-x-8 gap-y-10 lg:max-w-none lg:grid-cols-2 lg:gap-y-16">
            {features.map((feature) => (
              <div key={feature.name} className="relative pl-16">
                <dt className="text-base font-semibold leading-7 text-gray-900">
                  <div className="absolute left-0 top-0 flex h-10 w-10 items-center justify-center rounded-lg bg-black">
                    {/* Icon visualization */}
                    <span className="text-white font-bold">{feature.name[0]}</span>
                  </div>
                  {feature.name}
                </dt>
                <dd className="mt-2 text-base leading-7 text-gray-600">{feature.description}</dd>
              </div>
            ))}
          </dl>
        </div>
      </div>
    </div>
  );
}
